...
//
// test_llama_infer_gqa.c — verify GQA (Grouped-Query Attention) in slate's
// LLaMA inference path.  Uses a synthetic GGUF where n_q_heads = 8 and
// n_kv_heads = 2 (4x reduction, matching LLaMA-3-8B's pattern), and
// compares last-position logits to a numpy reference that does the
// same GQA forward.
...
int main(void) {
    slate_gguf_t* g = slate_gguf_open("/tmp/slate_tiny_llama_gqa.gguf");
    if (!g) { puts("gguf open FAIL — run tools/make_tiny_llama_gqa_gguf.py"); return 1; }
    slate_llama_t* m = slate_llama_open(g);
    if (!m) { puts("llama_open FAIL"); return 1; }
    const slate_llama_config_t* cfg = slate_llama_config(m);
    printf("GQA LLaMA: V=%d D=%d L=%d n_heads=%d n_kv_heads=%d head_dim=%d\n",
            cfg->vocab, cfg->d_model, cfg->n_layers,
            cfg->n_heads, cfg->n_kv_heads, cfg->head_dim);
    if (cfg->n_heads != 8 || cfg->n_kv_heads != 2) {
        puts("FAIL: expected n_heads=8, n_kv_heads=2"); return 1;
    }

    FILE* fp = fopen("/tmp/slate_tiny_llama_gqa_ref.bin", "rb");
